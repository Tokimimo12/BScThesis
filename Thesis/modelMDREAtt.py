...
def get_probabilities(model_output):
...
def train_model(model, train_loader, dev_loader, MAX_EPOCH=1000, patience=8, num_trials=3, grad_clip_value=1.0):
    ...
    for e in range(MAX_EPOCH):
        ...
        if valid_loss <= best_valid_loss:
            ...
            torch.save(model.state_dict(), 'modelmdreattention.std')
            torch.save(optimizer.state_dict(), 'optimmdreattention.std')
            curr_patience = patience
        else:
            curr_patience -= 1
            if curr_patience <= -1:
                ...
                num_trials -= 1
                curr_patience = patience
                model.load_state_dict(torch.load('modelmdreattention.std'))
                optimizer.load_state_dict(torch.load('optimmdreattention.std'))
                lr_scheduler.step()
...
def plot_sentiment_histogram(csv_file):
    ...
    sentiment_count = {category: sentiment_count.get(category, 0) for category in categories_order}
    ...
    plt.tight_layout()
    plt.savefig('plots/histogram2.png')
...
#     for i in range(7):
#         # Convert categorical labels to binary values for the One-vs-Rest strategy
...
#     # Random chance line for reference
#     plt.plot([0, 1], [0, 1], 'k--')  
#     plt.xlim([0.0, 1.0])
#     plt.ylim([0.0, 1.05])
...
def test_model_classification(model, test_loader):
    ...
    with torch.no_grad():
        ...
        for batch in test_loader:
            ...
            y_tilde = model(t, a, l)
            print(f"Model output y_tilde shape: {y_tilde.shape}")
            # y_tilde_prob = torch.softmax(y_tilde, dim=1)  # Assuming multi-class classification with softmax output
            # print(f"Model output y_tilde_prob shape: {y_tilde_prob.shape}")
            ...
            # y_pred_prob = y_pred
            # y_pred_prob.append(y_tilde_prob.detach().cpu().numpy())
