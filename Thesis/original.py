...
num_segments = len(dataset[visual_field].keys())  # Assuming all fields have the same segment keys
...
segment_lengths_visual = [dataset[visual_field][seg]['features'].shape[0] for seg in dataset[visual_field].keys()]
segment_lengths_acoustic = [dataset[acoustic_field][seg]['features'].shape[0] for seg in dataset[acoustic_field].keys()]
segment_lengths_text = [dataset[text_field][seg]['features'].shape[0] for seg in dataset[text_field].keys()]
...
for seg in dataset[visual_field].keys():  # Assuming all modalities have the same segment keys
    visual_length = dataset[visual_field][seg]['features'].shape[0]
    acoustic_length = dataset[acoustic_field][seg]['features'].shape[0]
    text_length = dataset[text_field][seg]['features'].shape[0]
    
    print(f"Segment {seg}: Visual length = {visual_length}, Acoustic length = {acoustic_length}, Text length = {text_length}")
    print(f"Text: {dataset[text_field][seg]['features']}")
...
video_ids = set(seg.split("[")[0] for seg in dataset[visual_field].keys())

# Count the number of unique video IDs
num_unique_ids = len(video_ids)

# Count segments for each video ID
video_segment_counts = {video_id: sum(seg.startswith(video_id) for seg in dataset[visual_field].keys()) for video_id in video_ids}
...
for segment in dataset[label_field].keys():
    ...
    # 
    for i, word in enumerate(_words):
        print(f"Processing segment {segment}: word[0] = {word[0]}")  # Log the word being processed

        if word[0] != b'sp':  # Skip pause fillers
            decoded_word = word[0].decode('utf-8')  # Decode the byte string to a regular string
            word_id = word2id[decoded_word]  # Map the decoded word to its ID
            words.append(word_id)  # Add the word ID to the list
            print(f"Added to words: {decoded_word} (ID: {word_id})")  # Log the added word
            visual.append(_visual[i, :])  # Append corresponding visual features
            acoustic.append(_acoustic[i, :])  # Append corresponding acoustic features
        else:
            print(f"Skipping pause filler: {word[0]}")  # Log skipped pause fillers

    
    visual_length = len(visual)
    acoustic_length = len(acoustic)
    text_length = len(words)

    # Print the segment details
    print(f"\n-----------\nSegment {seg}: Visual length = {visual_length}, Acoustic length = {acoustic_length}, Text length = {text_length}")
    print(f"Words: {words} \n -------\n")
    ...
    std_dev_acoustic[std_dev_acoustic == 0] = 0
    
    acoustic = np.nan_to_num((acoustic - acoustic_mean) / (std_dev_acoustic + EPS))
    
    print(f"Acoustic range after normalization: min={np.min(acoustic)}, max={np.max(acoustic)}")
    ...
    visual[:, std_dev_visual.flatten() == 0] = 0
    
    # print(f"visual: {visual}")
    # visual = np.nan_to_num((visual - visual.mean(0, keepdims=True)) / (EPS + np.std(visual, axis=0, keepdims=True)))
    print(f"Visual range after normalization: min={np.min(visual)}, max={np.max(visual)}\nxxxxx\n")
    ...
    if np.any(np.isnan(std_dev_visual)):
        print("Error: std_dev_acoustic contains NaNs.")
        
    if np.any(np.isnan(visual)):
        print("Error: NaN values in normalized acoustic data.")
    if np.any(np.isinf(visual)):
        print("Error: Infinite values in normalized acoustic data.")
...
print(f"Total number of {count} words")
...
for e in range(MAX_EPOCH):
    ...
    for batch in train_iter:
        ...
        if CUDA:
            ...
            y = y.cuda()
            l = l.cuda()
        print(f"t: {t.shape}, v: {v.shape}, a: {a.shape}, y: {y.shape}, l: {l.shape}")
